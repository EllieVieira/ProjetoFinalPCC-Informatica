...
class RespostaDAO {
    ...
    public function buscarDadosR( $idDiscursao ) {
        $res = array();
        $cmd = $this->pdo->prepare( "SELECT respostas.id, respostas.descricao, respostas.data, respostas.ativo, respostas.usuarios_id, respostas.discursao_id, usuarios.nome, respostas.pontuacao, respostas.votos FROM usuarios inner join respostas on respostas.usuarios_id = usuarios.id where respostas.discursao_id = :idDiscursao and respostas.ativo = 1 " );
